...
import telebot
import sqlite3
import threading  # <-- здесь один раз
import atexit
from datetime import datetime
import time
import schedule
from threading import Thread
from flask import Flask  # <-- Flask добавляем сюда
...
def show_stats(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        try:
            ...
            cursor.execute("SELECT COUNT(*) FROM users WHERE tariff = 'читатель'")
            readers = cursor.fetchone()[0]
            
            cursor.execute("SELECT COUNT(*) FROM users WHERE tariff = 'участник'")
            ...
            cursor.execute("SELECT COUNT(DISTINCT user_id) FROM channel_messages")
            active_in_channel = cursor.fetchone()[0]
            
            stats = f"📊 *СТАТИСТИКА БОТА:*\n\n"
            stats += f"👥 Всего пользователей: {total}\n"
            stats += f"👀 Получили урок: {clicked}\n"
            stats += f"🎯 Выбрали тариф: {with_tariff}\n"
            stats += f"💰 Оплатили (в клубе): {paid}\n"
            stats += f"📖 Читатели: {readers}\n"
            stats += f"💎 Участники: {members}\n"
            stats += f"💵 Общий доход: {total_income}₽\n"
            stats += f"💬 Сообщений в канале: {total_messages}\n"
            stats += f"👤 Уникальных в канале: {active_in_channel}\n"
            
            if total > 0:
                conv_to_paid = paid/total*100 if total > 0 else 0
...
def send_daily_report_auto():
    """Автоматическая отправка ежедневного отчета"""
    try:
        conn, cursor = get_db_connection()
        
        # Проверяем, были ли сообщения за последние 24 часа
        cursor.execute("SELECT COUNT(*) FROM channel_messages WHERE date >= datetime('now', '-24 hours')")
        message_count = cursor.fetchone()[0]
        ...
        if message_count > 0:
            ...
            report += f"*За последние 24 часа:*\n"
            report += f"👤 Активных пользователей: {user_count}\n"
            report += f"💬 Сообщений: {message_count}\n\n"
            report += f"Для подробного отчета используйте /report"
            
            bot.send_message(ADMIN_ID, report, parse_mode='Markdown')
        else:
            bot.send_message(ADMIN_ID, "📭 За последние 24 часа в канале сообщений не было.")
            
    except Exception as e:
        print(f"❌ Ошибка в авто-отчете: {e}")

def run_scheduler():
    """Запускает планировщик для автоматических отчетов"""
    # Настраиваем время отправки (например, каждый день в 20:00)
    schedule.every().day.at("20:00").do(send_daily_report_auto)
    
    print("⏰ Планировщик отчетов запущен (ежедневно в 20:00)")
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("📊 НОВЫЕ ФУНКЦИИ ОТЧЕТОВ:")
    print("/report - активность за 24 часа")
    print("/report7 - активность за 7 дней")
    print("/activity - меню отчетов")
    print("/stats - общая статистика")
    print("/list - список пользователей")
    print("=" * 50)
    print("💬 АВТОМАТИЧЕСКИЙ МОНИТОРИНГ:")
    print("✓ Бот отслеживает все сообщения в канале")
    print("✓ Сохраняет кто, что и когда написал")
    print("✓ Привязывает тариф пользователя")
    print("✓ Авто-отчет каждый день в 20:00")
    print("=" * 50)
